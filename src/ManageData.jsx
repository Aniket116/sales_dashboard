...
const ManageDataPage = () => {
    ...
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
    // --- NEW: State to control the upload modal's visibility ---
    const [isModalOpen, setIsModalOpen] = useState(false);

    // const fetchRecentRecords = useCallback(async () => {
    //     setLoading(true);
    //     // --- FIX: Fetch the new columns to display in the table ---
    //     const { data, error } = await supabase
    //         .from('new_sales_data')
    //         .select('Date, "Product ID", Category, Region, "Inventory Level", "Units Sold", Revenue, Discount')
    //         .order('id', { ascending: false }) // Order by ID to see the absolute latest inserts
    //         .limit(25);
    ...
    const handleUploadSuccess = () => {
        fetchRecentRecords(); // Refresh the table
        setIsModalOpen(false); // Close the modal
    };

    return (
        <div className="space-y-6">
            {/* --- FIX: Updated header with Flexbox for alignment --- */}
            <header className="flex justify-between items-center">
                <div>
                    <h2 className="text-3xl font-bold text-white text-left">Data Management</h2>
                    <p className="text-gray-400 mt-1 text-left">View recent records or upload new data via CSV.</p>
                </div>
                <button 
                    onClick={() => setIsModalOpen(true)}
                    className="bg-cyan-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-cyan-700 flex items-center space-x-2"
                >
            ...
            {/* --- FIX: The table now takes the full width --- */}
            <div className="w-full">
                <RecentRecordsTable records={recentRecords} loading={loading} error={error} />
            </div>

            {/* --- NEW: The upload functionality is now in a modal --- */}
            <UploadModal 
                isOpen={isModalOpen}
...
// --- NEW: Modal Component for CSV Upload ---
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                if (uploadError) {
                    ...
                    setMessage({ type: 'success', text: `Successfully uploaded ${data.length} records!` });
                    onUploadSuccess(); // This will also close the modal
                }
                setIsUploading(false);
                setFile(null);
            }
            ...
            <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
                ...
                <div className="space-y-4">
                    <p className="text-sm text-gray-400">
                        Select a CSV file with your sales data. Ensure the column headers match the database schema.
                    </p>
                    <input type="file" accept=".csv" onChange={handleFileChange} className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-cyan-50 file:text-cyan-700 hover:file:bg-cyan-100"/>
                    <button onClick={handleUpload} disabled={!file || isUploading} className="w-full bg-blue-600 text-white font-bold py-2 px-4 rounded hover:bg-blue-700 disabled:bg-gray-500">
    ...
    <div className="bg-gray-800 p-6 rounded-lg shadow-md">
        ...
        <div className="overflow-auto max-h-[500px]">
            <table className="min-w-full">
                <thead className="bg-gray-700 sticky top-0">
                    <tr>
                        {/* --- FIX: Added new columns to the table header --- */}
                        {['Date', 'Product ID', 'Category', 'Region', 'Inventory Level', 'Units Sold', 'Revenue', 'Discount'].map(h => (
                            <th key={h} className="py-2 px-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">{h}</th>
                        ))}
                    </tr>
                </thead>
                <tbody className="bg-gray-800">
                    {loading ? (
                        ...
                        <tr><td colSpan="8" className="text-center py-4 text-red-400">{error}</td></tr>
                    ) : (
                        records.map((row, i) => (
                            <tr key={i} className="border-b border-gray-700 last:border-b-0">
                                {/* --- FIX: Added new cells to render the new data --- */}
                                <td className="py-2 px-3 text-sm text-gray-200">{row.Date}</td>
                                <td className="py-2 px-3 text-sm text-gray-200">{row['Product ID']}</td>
                                <td className="py-2 px-3 text-sm text-gray-200">{row.Category}</td>
                                <td className="py-2 px-3 text-sm text-gray-200">{row.Region}</td>
                                <td className="py-2 px-3 text-sm text-gray-200">{row['Inventory Level']}</td>
                                <td className="py-2 px-3 text-sm text-gray-200">{row['Units Sold']}</td>
                                <td className="py-2 px-3 text-sm text-gray-200">${row.Revenue?.toLocaleString()}</td>
                                <td className="py-2 px-3 text-sm text-gray-200">{row.Discount}%</td>
                            </tr>
                        ))
                    )}
                </tbody>
