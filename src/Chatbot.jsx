...
//   // For security, move this key to a .env file: VITE_GEMINI_API_KEY=YOUR_KEY
...
//   // --- Main logic for handling a user's message ---
...
//     try {
//       // --- Step 1: LLM generates a "plan" to answer the question ---
//       const queryPlanningPrompt = `
//         You are a data analysis planner. Your job is to understand a user's question and create a JSON plan to fetch and process data from a Supabase table named "new_sales_data".

//         Available columns: "Date", "Store ID", "Product ID", "Category", "Region", "Inventory Level", "Units Sold", "Price", "Discount", "Demand", "Revenue", "Profit_Margin".
        
//         IMPORTANT: When referencing a column name with a space in it, you MUST enclose it in double quotes (e.g., "\\"Units Sold\\"").
//         IMPORTANT: Assume the current date is August 31st, 2025 for all inventory-related queries.
//         Your JSON plan must have:
//         - "select": A string of the RAW column names needed for the calculation (e.g., "\\"Units Sold\\", Revenue").
//         - "filters": An array of filter objects { "column", "operator", "value" }.
//         - "operation": An object describing the calculation to perform on the fetched data, with "method" ("SUM", "AVG", "COUNT") and "column" (the column to perform the operation on, e.g., "Units Sold").

//         If the question is general conversation, return {"isGeneral": true}.

//         Example 1: "total sales for toys in the north region"
//         Result: {"select": "\\"Units Sold\\"", "filters": [{"column": "Category", "operator": "eq", "value": "Toys"}, {"column": "Region", "operator": "eq", "value": "North"}], "operation": {"method": "SUM", "column": "Units Sold"}}

//         Example 2: "what is the average inventory level for electronics"
//         Result: {"select": "\\"Inventory Level\\"", "filters": [{"column": "Category", "operator": "eq", "value": "Electronics"}], "operation": {"method": "AVG", "column": "Inventory Level"}}
        
//         Example 3: "how many sales were there for clothing"
//         Result: {"select": "*", "filters": [{"column": "Category", "operator": "eq", "value": "Clothing"}], "operation": {"method": "COUNT", "column": null}}
...
//       } else {
//         // --- Step 2: Fetch the raw data from Supabase based on the plan ---
...
//             // Use bracket notation for the column name to handle spaces correctly
...
//           throw new Error("No data found for your query.");
//         }

//         // --- Step 3: Perform the calculation in JavaScript ---
//         let calculatedResult;
//         const targetColumn = plan.operation.column;

//         switch (plan.operation.method) {
//           case 'SUM':
//             calculatedResult = data.reduce((sum, row) => sum + (parseFloat(row[targetColumn]) || 0), 0);
//             break;
//           case 'AVG':
//             const sum = data.reduce((s, row) => s + (parseFloat(row[targetColumn]) || 0), 0);
//             calculatedResult = sum / data.length;
//             break;
//           case 'COUNT':
//             calculatedResult = data.length;
//             break;
//           default:
//             calculatedResult = data;
//         }

//         // --- Step 4: Send the final number to the LLM for summarization ---
//         const summarizationPrompt = `
//           You are a helpful data analyst. The user asked a question, and the calculated answer is provided below. Present this answer in a concise, friendly, and natural language sentence.
...
//         <div className="flex items-center justify-between p-3 bg-gradient-to-r from-purple-600 to-indigo-700 text-white rounded-t-lg shadow-md">
//           <div className="flex items-center">
//             <div className="bg-purple-300 w-8 h-8 rounded-full flex items-center justify-center mr-2"><span className="text-purple-800 text-lg font-bold">🤖</span></div>
//             <div><p className="font-semibold text-lg leading-tight">DataBot</p><p className="text-xs text-purple-200">Online Now</p></div>
//           </div>
//           <button onClick={() => setIsOpen(false)} className="text-white hover:text-purple-200 text-2xl leading-none">&times;</button>
//         </div>
//         <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-gray-50">
//           {messages.map((msg, index) => (
//             <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
//               <div className={`px-4 py-2 rounded-xl max-w-[75%] break-words ${msg.role === 'user' ? 'bg-indigo-500 text-white rounded-br-none' : 'bg-gray-200 text-gray-800 rounded-bl-none'}`}>{msg.text}</div>
//             </div>
//           ))}
...
//           <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Ask about your data..." className="flex-grow p-2 border border-gray-300 rounded-l-full focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm text-gray-900" disabled={isLoading}/>
//           <button type="submit" className="bg-indigo-600 text-white px-4 py-2 rounded-r-full hover:bg-indigo-700 transition-colors duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed text-sm" disabled={isLoading}>Send</button>
//         </form>
//       </div>
//       <button onClick={() => setIsOpen(!isOpen)} className="fixed bottom-4 right-4 w-14 h-14 bg-gradient-to-r from-purple-600 to-indigo-700 text-white rounded-full shadow-lg flex items-center justify-center text-3xl cursor-pointer hover:scale-110 transition-transform duration-200 ease-in-out z-50 focus:outline-none focus:ring-4 focus:ring-indigo-300" aria-label={isOpen ? "Close chat" : "Open chat"}>{isOpen ? '✕' : '💬'}</button>
//     </>
//   );
// };
